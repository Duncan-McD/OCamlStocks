let ask_for_stock () =
  print_endline
    "Enter a stock ticker to retrieve its reccomendation rating from Yahoo \
     Finance - Example: GME not \"GME\"";
...
let rec main () =
  ...
  let stock_is_none = match stock with None -> true | _ -> false in
  print_endline (ticker ^ " Recommendation Rating: ");
  print_float
    (if stock_is_none then 3.
    else stock |> Stockdata.require ticker |> Stockdata.rating);
  print_endline "";
  print_endline (ticker ^ " History Score: ");
  print_float
    (if stock_is_none then Parser.history_score None
    else Parser.history_score stock);
  print_endline "";
